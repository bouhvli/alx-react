...
export const getNotifitcations = (state) => state.get("notifications");
...
export const getUnreadNotificationsByType = createSelector(
  ...
  (notifications) => {
    ...
    if (messages) {
      ...
      if (filter === "URGENT") {
        filtered = messages.valueSeq()
          .filter(
            (value) => value
              .get("isRead") === false && value.get("type") === "urgnet"
          );
      } else {
        filtered = messages
          .valueSeq()
          .filter(
            (value) => value.get("isRead") === false
          );
      }
      return filtered;
    }
